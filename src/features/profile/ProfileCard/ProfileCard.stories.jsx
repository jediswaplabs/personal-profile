...
const defaultProfileState = { address: '0x00000000000000',
  readOnly: false };
...
NoAvatar.args = { storyTitle: 'No avatar',
  readOnly: false };
NoAvatar.decorators = [
  (Story) => {
    const MockStore = renderWithProviders({ preloadedState: { profile: defaultProfileState } });
...
NoAvatar.parameters = { msw: { handlers: [
  graphql.query('GetUser', (req, res, ctx) => res(
    ctx.delay(ARTIFICIAL_DELAY_MS),
...
ReadOnly.parameters = { msw: { handlers: [
  graphql.query('GetUser', (req, res, ctx) => res(
    ctx.delay(ARTIFICIAL_DELAY_MS),
    ctx.data(defaultUserData),
  )),
...
const stories = { title: 'Components/ProfileCard',
